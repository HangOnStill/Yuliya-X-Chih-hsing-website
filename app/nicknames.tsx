...
export default function Nicknames({active}:{active:boolean}){
 ...
 return <section>
  <div className="section-top"><div><p className="eyebrow">OUR PRIVATE DICTIONARY · 暱稱小本</p><h2>Only you call me that.</h2><p className="quiet">那些只有彼此懂的稱呼，和一想起來就會笑的瞬間。</p></div><EditButton onClick={()=>setEditing('new')}><Plus size={17}/>Add a nickname · 新增暱稱</EditButton></div>
  ...
  <div className="nickname-grid">{shown.map(e=><article className="v5-card nickname-card" key={e.id}>
   ...
   <div className="nickname-actions"><Button variant="outline" onClick={()=>setReading(e)}><BookOpen size={16}/>Read story</Button><EditButton variant="ghost" size="icon" onClick={()=>setEditing(e)} aria-label={'Edit nickname '+e.data.name}><Pencil size={16}/></EditButton><EditButton variant="ghost" size="icon" onClick={()=>setRemoving(e)} aria-label={'Remove nickname '+e.data.name}><Trash2 size={16}/></EditButton></div>
  </article>)}</div>
  {loaded&&!error&&!shown.length&&<div className="v5-empty"><Smile size={34}/><p>{nicknames.length?'還找不到符合的暱稱，換個人物或關鍵字試試。':'從第一個只有你會叫的名字開始。'}</p>{!nicknames.length&&<EditButton variant="outline" onClick={()=>setEditing('new')}>Keep our first nickname · 收藏第一個暱稱</EditButton>}</div>}
  ...
  <Dialog open={!!reading} onOpenChange={v=>{if(!v)setReading(null);}}><DialogContent className="nickname-read-dialog">{reading&&<><DialogTitle>「{reading.data.name}」</DialogTitle><DialogDescription>A name for {reading.data.person}{reading.data.since?' · Since '+reading.data.since:''}</DialogDescription><div className="nickname-story"><h3>How it began · 緣起</h3><p>{reading.data.origin||'還沒寫下緣起。'}</p><h3>A moment we still laugh about · 搞笑瞬間</h3><p>{reading.data.funnyMoment||'還有許多值得收藏的笑聲。'}</p></div><EditButton variant="outline" onClick={()=>{setEditing(reading);setReading(null);}}><Pencil size={16}/>Add to our story · 編輯故事</EditButton></>}</DialogContent></Dialog>
...
function NicknameEditor({entry,initialPerson,close,saved}:{entry?:Nickname;initialPerson:typeof people[number];close:()=>void;saved:()=>void}){
 ...
 return <><Dialog open onOpenChange={v=>{if(!v)leave();}}><DialogContent className="nickname-edit-dialog"><DialogTitle>{entry?'A little more to the story · 編輯暱稱':'A name only we know · 新增暱稱'}</DialogTitle><DialogDescription>Keep the name, how it began, and the moment that still makes you smile.</DialogDescription><form className="nickname-form" onSubmit={save}>
  ...
  <div className="dialog-actions"><Button type="button" variant="outline" disabled={busy} onClick={leave}>Cancel · 取消</Button><EditButton type="submit" disabled={busy}>{busy?'Saving…':'Keep this nickname · 收藏暱稱'}</EditButton></div>
