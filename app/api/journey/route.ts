...
export async function POST(req:Request){try{if(req.method!=='POST')throw new HttpError(405,'Method not allowed.');sameOrigin(req);const v=z.object({index:z.number().int().min(0).max(5),answer:z.string().max(500)}).parse(await body(req));const persistent=canEdit(req),user=persistent?identity(req):null,unlocked=user?await progress(user):v.index;if(v.index>unlocked)throw new HttpError(400,'Open the earlier memory first.');const q=(await resolvedQuestions())[v.index].data,answer=normalized(v.answer),correct=!!answer&&matchesAnswer(v.answer,q.answer);if(correct&&user)await db().prepare('INSERT INTO journey(user_id,unlocked) VALUES(?,?) ON CONFLICT(user_id) DO UPDATE SET unlocked=MAX(journey.unlocked,excluded.unlocked)').bind(user,v.index+1).run();return json({correct,feedback:correct?q.successFeedback:answer?q.wrongFeedback:q.emptyFeedback,unlocked:correct?Math.max(unlocked,v.index+1):unlocked,temporary:!persistent});}catch(e){return apiError(e);}}
