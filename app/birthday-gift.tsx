...
function BirthdayGiftContent({questions,canEdit=false,signInUrl,signedIn=false}:{questions:Question[];canEdit?:boolean;signInUrl:string;signedIn?:boolean}){
 const {entries:poetryEntries,error:poetryError,refresh:refreshPoetry}=useEntries(canEdit);
 ...
 return <GiftAccess.Provider value={canEdit}><SidebarProvider open={prefs.sidebarOpen} onOpenChange={sidebarOpen=>setPrefs(p=>({...p,sidebarOpen}))} className="v5-app" style={{"--sidebar-width":"15rem"} as CSSProperties}><GiftSidebar tab={tab} navigate={navigate} settings={()=>setSettings(true)}/><div className="gift-app" onErrorCapture={e=>{if(canEdit&&['IMG','VIDEO','AUDIO'].includes((e.target as HTMLElement).tagName))void request('/api/journey').catch(()=>{});}}>
  ...
  <main id="main" className="gift-shell">
   <div className="view-stack">
    ...
    {canEdit?<><section hidden={tab!=='memories'} className="tab-panel"><MemoryArchive photos={photos.filter(p=>p.kind==='photo')} loading={photoLoading} error={photoError} refresh={refreshPhotos}/></section>
