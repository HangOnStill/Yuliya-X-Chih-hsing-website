...
import {LetterChapterHeading} from './letter-chapter-heading';
import {LetterTitleSettings} from './letter-title-settings';
import {LetterQuestionSettings} from './letter-question-settings';
import {resolveLetterTitles} from '@/lib/letter-titles';
import {archiveDeniedEvent} from '@/lib/archive-client';
...
import PoetryLibrary,{LetterPoemSettings} from './poetry-library';
...
type GiftProps={questions:Question[];canAccessArchive?:boolean;signInUrl:string;signedIn?:boolean};
export default function BirthdayGift({canAccessArchive=false,...props}:GiftProps){
 const [revoked,setRevoked]=useState(false);
 useEffect(()=>{const deny=()=>setRevoked(true);window.addEventListener(archiveDeniedEvent,deny);return()=>window.removeEventListener(archiveDeniedEvent,deny);},[]);
 const allowed=canAccessArchive&&!revoked;
 // Remount on capability loss: discard all private collections, drafts, media
 // references and child state, and run recording/export cleanup on unmount.
 return <BirthdayGiftContent key={allowed?'member':'guest'} {...props} canEdit={allowed}/>;
}
function BirthdayGiftContent({questions:initialQuestions,canEdit=false,signInUrl,signedIn=false}:{questions:Question[];canEdit?:boolean;signInUrl:string;signedIn?:boolean}){
 const [questions,setQuestions]=useState(initialQuestions),[questionError,setQuestionError]=useState('');
 const refreshQuestions=useCallback(async()=>{try{const r=await request('/api/journey/public');setQuestions(r.questions);setQuestionError('');}catch(e){setQuestionError((e as Error).message);}},[]);
 useEffect(()=>{void refreshQuestions();const update=()=>void refreshQuestions();window.addEventListener('focus',update);window.addEventListener('pageshow',update);window.addEventListener('yc-archive-change',update);return()=>{window.removeEventListener('focus',update);window.removeEventListener('pageshow',update);window.removeEventListener('yc-archive-change',update);};},[refreshQuestions]);
 const {entries:poetryEntries,error:poetryError,refresh:refreshPoetry,loaded:poetryLoaded}=useEntries(canEdit);
 const titles=resolveLetterTitles(poetryEntries,canEdit),themes=titles.map(t=>t.chineseSubtitle);
 ...
 const refreshJourney=useCallback(async()=>{if(!canEdit)return;try{const r=await request('/api/journey');if(!r.temporary){setUnlocked(r.unlocked);setIndex(i=>Math.min(i,r.unlocked,5));}setJourneyError('');}catch(e){setJourneyError((e as Error).message);}},[canEdit]);
 ...
 return <GiftAccess.Provider value={canEdit}><SidebarProvider open={prefs.sidebarOpen} onOpenChange={sidebarOpen=>setPrefs(p=>({...p,sidebarOpen}))} className="v5-app" style={{"--sidebar-width":"15rem"} as CSSProperties}><GiftSidebar tab={tab} navigate={navigate} settings={()=>setSettings(true)}/><div className="gift-app" onErrorCapture={e=>{if(canEdit&&['IMG','VIDEO','AUDIO'].includes((e.target as HTMLElement).tagName))void request('/api/journey').catch(()=>{});}}>
  ...
  {!canEdit&&<div className="guest-note"><span>Enjoy the letter. Our personal archive is private.</span>{!signedIn&&<a href={signInUrl} target="_top">Sign in to access our archive</a>}</div>}
   ...
   <div className="view-stack">
    <section hidden={tab!=='letter'} className="tab-panel letter-tab">
     ...
     {opened&&<section ref={journeyRef} id="journey" className="letter-journey" aria-label="Our six memories">
      ...
      {questionError&&<div className="error-banner" role="alert">{questionError}<Button variant="outline" onClick={refreshQuestions}>Reload questions</Button></div>}
      <div className="chapter-nav" aria-label="Memory chapters">{questions.map((q,i)=><button key={q.index} disabled={i>unlocked||busy} onClick={()=>setIndex(i)} aria-current={i===index?'step':undefined} aria-label={`${titles[i].mainTitle}${i>unlocked?' · locked':''}`} className={i===index?'current':i<unlocked?'done':''}><span>{i<unlocked?<Check size={16}/>:String(i+1).padStart(2,'0')}</span><small lang="zh-Hant">{themes[i]}</small></button>)}</div><Progress className="journey-progress" value={unlocked/6*100} aria-label={`${unlocked} of 6 memories unlocked`}/>
      <article className="journey-spread" key={index}><div className="journey-photo-side"><div className="journey-photo">{photo?<img src={'/api/media/'+photo.id} alt={photo.title} decoding="async"/>:<div className="unfilled-memory"><span className="large-number">0{index+1}</span><ImagePlus size={25}/><p>A place for this memory</p><Button variant="outline" onClick={()=>navigate('memories')}>Choose a photograph</Button></div>}<span className="photo-counter">0{index+1} / 06</span>{poemVisible&&<div className="poetry-overlay" role="region" aria-label="Unlocked poem"><Poetry poem={poem} custom={!!customPoem}/><Button variant="outline" className="poetry-dismiss" onClick={()=>setPoemVisible(false)}>收卷 · Return to photo</Button></div>}</div><p className="photo-caption">{photo?.note||photo?.title||titles[index].mainTitle}</p></div>
       <div className="question-side"><LetterChapterHeading title={titles[index]}/><p className="question-text">{questions[index].text}</p>{!solved?<form onSubmit={checkAnswer}><Label htmlFor="answer">Your answer</Label><div className="answer-row"><Input ref={answerRef} id="answer" value={answer} onChange={e=>{setAnswer(e.target.value);setFeedback('');}} placeholder="Type it here…" autoComplete="off" maxLength={500} disabled={busy||!!questionError}/><Button type="submit" disabled={busy||!!questionError}>{busy?'Checking…':'Unlock'}<LockKeyhole size={15}/></Button></div></form>:<div className="unlocked-label"><Check size={19}/> This memory is yours.</div>}
       <div className={'answer-feedback '+(correct||solved?'success':'')} aria-live="polite">{feedback}</div>
    ...
    <section hidden={tab!=='keepsakes'} className="tab-panel"><Keepsakes active={tab==='keepsakes'} photos={photos}/></section></>:tab!=='letter'&&<section className="tab-panel"><p className="quiet">This space is private to the two archive members.</p><a href={signInUrl} target="_top">Sign in with an invited account</a></section>}
   </div>
  </main>
  ...
  <Dialog open={settings} onOpenChange={setSettings}><DialogContent className="settings-dialog" onOpenAutoFocus={e=>{e.preventDefault();document.querySelector<HTMLButtonElement>('.settings-dialog [data-slot=dialog-close]')?.focus({preventScroll:true});}}><DialogTitle>Make the letter your own</DialogTitle><DialogDescription>Choose a whole poetry set, or mix one verse for each memory. These display choices are remembered on this device.</DialogDescription><div className="settings-switches"><div className="effect-switch"><Label htmlFor="envelope-setting">Envelope opening</Label><Switch id="envelope-setting" checked={prefs.envelope} onCheckedChange={v=>setPrefs(p=>({...p,envelope:v}))}/></div><div className="effect-switch"><Label htmlFor="poem-setting">Poetry animation</Label><Switch id="poem-setting" checked={prefs.poetry} onCheckedChange={v=>setPrefs(p=>({...p,poetry:v}))}/></div></div><Choice label="Apply a full set" value={prefs.sets.every(s=>s===prefs.sets[0])?prefs.sets[0]:'mixed'} options={[{value:'mixed',label:'Mix & match · 自選'},...poemSets.map(s=>({value:s.id,label:s.label}))]} onChange={v=>{if(v!=='mixed')setPrefs(p=>({...p,sets:Array(6).fill(v)}));}}/>
   ...
   {canEdit&&poetryLoaded&&<LetterTitleSettings entries={poetryEntries} error={poetryError} refresh={refreshPoetry}/>}
   {canEdit&&settings&&<LetterQuestionSettings/>}
   {canEdit&&<LetterPoemSettings entries={poetryEntries} error={poetryError} refresh={refreshPoetry}/>}
   <details className="poem-recordings"><summary>Read a poem aloud · 詩詞朗讀</summary><Choice label="Poem to record" value={String(recordPoem)} options={themes.map((label,i)=>({value:String(i),label:`0${i+1} · ${label}`}))} onChange={v=>setRecordPoem(Number(v))}/>{canEdit&&<AudioNotes key={'poem-'+recordPoem} parentType="poem" parentId={String(recordPoem)}/>}</details>
...
function Poetry({poem,custom=false}:{poem:Poem;custom?:boolean}){return <div className={'poetry-content'+(custom?' custom-poetry':'')} lang="zh-Hant"><div className="poetry-lines">{poem.lines.map((line,n)=><p key={n} className="poetry-line">{[...line].map((c,i)=><span key={i} style={{'--delay':`${n*.25+i*.075}s`} as CSSProperties}>{c}</span>)}</p>)}</div><p className="poetry-source">{poem.source}</p></div>;}
