...
import {canEdit,trustedIdentity} from './access';
...
export function identity(req:Request){const user=trustedIdentity(req.headers);if(!user)throw new HttpError(401,'Sign in to access the private archive.');return user.userId;}
export function requireArchiveMember(req:Request){
 const userId=identity(req);
 if(!canEdit(req))throw new HttpError(403,'This account cannot access the private archive.');
 return userId;
}
export function authorize(req:Request){requireArchiveMember(req);sameOrigin(req);}
...
export function apiError(e:unknown){if(e instanceof HttpError)return json({error:e.message},e.status);if(e instanceof ZodError)return json({error:e.issues[0]?.message||'Please check the fields.'},400);console.error('Archive request failed',e instanceof Error?e.message:'unknown');return json({error:'We could not complete that request. Your input has been kept; please try again.'},503);}
// Every archive route is private by default, for reads as well as writes.
export function api(fn:(req:Request,ctx:any)=>Promise<Response>){return async(req:Request,ctx:any)=>{try{authorize(req);return await fn(req,ctx);}catch(e){return apiError(e);}};}
